...
export default function SlidingTabDialog({
    open,
    onClose,
    slidingTabId,
    onSuccess,
}: SlidingTabDialogProps) {
    ...
    // 获取滑动标签详情（编辑模式）
    const { data: slidingTabData } = api.slidingTab.getById.useQuery(
        { id: slidingTabId! },
        { enabled: !!slidingTabId }
    );

    // 新增滑动标签
    const createMutation = api.slidingTab.create.useMutation({
        onSuccess: () => {
            showSuccessToast('新增滑动标签成功');
            onSuccess();
            onClose();
        },
        onError: (error) => {
    ...
    // 更新滑动标签
    const updateMutation = api.slidingTab.update.useMutation({
        onSuccess: () => {
            showSuccessToast('更新滑动标签成功');
            // 清除缓存，强制重新获取数据
            void utils.slidingTab.list.invalidate();
            void utils.slidingTab.getById.invalidate();
            onSuccess();
            onClose();
        },
        onError: (error) => {
    ...
    // 加载编辑数据
    useEffect(() => {
        if (open && slidingTabId && slidingTabData) {
            console.log('Loading sliding tab data:', slidingTabData);
            console.log('Channels from data:', slidingTabData.channels);
            setFormData({
                image: slidingTabData.image,
                text: slidingTabData.text,
                link: slidingTabData.link,
                channels: slidingTabData.channels || [],
            });
        } else if (open && !slidingTabId) {
            // 新增模式，重置表单
            console.log('New sliding tab mode, resetting form');
            resetForm();
        }
    }, [open, slidingTabId, slidingTabData]);
