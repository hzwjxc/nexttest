...
export default function ActiveUsersPage() {
    ...
    // 获取活跃用户数据
    const { data, isLoading } = api.reports.getActiveUsers.useQuery({
        page: currentPage,
    ...
    // 导出数据查询
    const { refetch: fetchExportData } = api.reports.exportActiveUsers.useQuery(
        {
            startDate: startDate || undefined,
            endDate: endDate || undefined,
        },
    ...
    const handleExport = async () => {
        try {
            ...
            const worksheetData = exportData.map((item: any) => ({
                ...
                '最后登录渠道': item.lastLoginChannel,
            }));

            // 创建工作表
            const ws = XLSX.utils.json_to_sheet(worksheetData);
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, '活跃用户');
            ...
            // 生成文件名
            const fileName = `活跃用户_${startDate || '全部'}_${endDate || '全部'}.xlsx`;
