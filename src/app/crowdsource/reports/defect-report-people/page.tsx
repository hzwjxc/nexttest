...
export default function DefectReportPeoplePage() {
    ...
    // 获取提报缺陷人数数据
    const { data, isLoading } = api.reports.getDefectReportPeople.useQuery({
        page: currentPage,
    ...
    // 导出数据查询
    const { refetch: fetchExportData } = api.reports.exportDefectReportPeople.useQuery(
        {
            startDate: startDate || undefined,
            endDate: endDate || undefined,
        },
    ...
    const handleExport = async () => {
        try {
            ...
            // 准备导出数据
            const headers = ['序号', '用户名', '所属机构', '手机号', '最后提报时间', '最后任务编号', '最后任务名称', '最后缺陷名称', '最后判定结果', '最后判定意见'];
            const csvRows = [
                headers.join(','),
                ...exportData.map((item: any) => [
                    ...
                    `"${item.lastTaskName}"`,
                    `"${item.lastDefectName}"`,
                    `"${item.lastJudgmentResult}"`,
                    `"${item.lastJudgmentOpinion}"`
                ].join(','))
            ];
            ...
            link.setAttribute(
                'download',
                `提报缺陷人数_${startDate || '全部'}_${endDate || '全部'}.csv`
            );
