...
export default function DefectReportCountPage() {
    ...
    // 获取提报缺陷人次数据
    const { data, isLoading } = api.reports.getDefectReportCount.useQuery({
        page: currentPage,
    ...
    // 导出数据查询
    const { refetch: fetchExportData } = api.reports.exportDefectReportCount.useQuery(
        {
            startDate: startDate || undefined,
            endDate: endDate || undefined,
        },
    ...
    const handleExport = async () => {
        try {
            ...
            // 准备导出数据
            const headers = ['序号', '用户名', '所属机构', '手机号', '提报时间', '任务编号', '任务名称', '缺陷名称', '判定结果', '判定意见'];
            const csvRows = [
                headers.join(','),
            ...
            link.setAttribute(
                'download',
                `提报缺陷人次_${startDate || '全部'}_${endDate || '全部'}.csv`
            );
