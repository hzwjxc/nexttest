...
export default function TaskAcceptancePage() {
    ...
    // 获取任务领取人次数据
    const { data, isLoading } = api.reports.getTaskAcceptance.useQuery({
        page: currentPage,
    ...
    // 导出数据查询
    const { refetch: fetchExportData } = api.reports.exportTaskAcceptance.useQuery(
        {
            startDate: startDate || undefined,
            endDate: endDate || undefined,
        },
    ...
    const handleExport = async () => {
        try {
            ...
            // 准备导出数据
            const headers = ['序号', '用户名', '所属机构', '手机号', '领取时间', '任务编号', '任务名称'];
            const csvRows = [
                headers.join(','),
            ...
            link.setAttribute(
                'download',
                `任务领取人次_${startDate || '全部'}_${endDate || '全部'}.csv`
            );
